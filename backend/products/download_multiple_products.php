...
try {
    ...
    $stmt = $pdo->prepare("SELECT id, name, tamil_name, cat, sub_cat, price, original_price, weight, unit_name, img_url, disabled, index_num AS `index`, offer FROM products WHERE id IN ($placeholders)");
    $stmt->execute($ids);
