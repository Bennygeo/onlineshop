...
try {
    ...
    // === REFUND STATS (TODAY & LIFETIME) ===
    $todayRefunds = 0;
    $todayRefundCount = 0;
    $totalRefunds = 0;
    $totalRefundCount = 0;

    // 1. Check Wallets table for credited refunds
    try {
        $stmtTodayWalRef = $pdo->prepare("SELECT COALESCE(SUM(amount), 0) as amt, COUNT(*) as cnt FROM wallets WHERE (description LIKE '%Refund%' OR description LIKE '%refund%') AND UPPER(type) = 'CREDIT' AND DATE(created_at) = ?");
        $stmtTodayWalRef->execute([$today]);
        $todayWalRefRow = $stmtTodayWalRef->fetch();
        $todayRefunds = floatval($todayWalRefRow['amt'] ?? 0);
        $todayRefundCount = intval($todayWalRefRow['cnt'] ?? 0);

        $stmtTotWalRef = $pdo->query("SELECT COALESCE(SUM(amount), 0) as amt, COUNT(*) as cnt FROM wallets WHERE (description LIKE '%Refund%' OR description LIKE '%refund%') AND UPPER(type) = 'CREDIT'");
        $totWalRefRow = $stmtTotWalRef->fetch();
        $totalRefunds = floatval($totWalRefRow['amt'] ?? 0);
        $totalRefundCount = intval($totWalRefRow['cnt'] ?? 0);
    } catch (Exception $e) {}

    // 2. Cross-check with orders table refund_amount
    try {
        $stmtOrdTotRef = $pdo->query("SELECT COALESCE(SUM(refund_amount), 0) as amt, COUNT(CASE WHEN refund_amount > 0 THEN 1 END) as cnt FROM orders WHERE refund_amount > 0");
        $ordTotRefRow = $stmtOrdTotRef->fetch();
        $ordTotAmt = floatval($ordTotRefRow['amt'] ?? 0);
        $ordTotCnt = intval($ordTotRefRow['cnt'] ?? 0);
        if ($ordTotAmt > $totalRefunds) {
            $totalRefunds = $ordTotAmt;
            $totalRefundCount = $ordTotCnt;
        }

        // Today's orders refund
        $stmtOrdTodayRef = $pdo->prepare("SELECT COALESCE(SUM(refund_amount), 0) as amt, COUNT(CASE WHEN refund_amount > 0 THEN 1 END) as cnt FROM orders WHERE refund_amount > 0 AND (DATE(created_at) = ? OR DATE(updated_at) = ?)");
        $stmtOrdTodayRef->execute([$today, $today]);
        $ordTodayRefRow = $stmtOrdTodayRef->fetch();
        $ordTodayAmt = floatval($ordTodayRefRow['amt'] ?? 0);
        $ordTodayCnt = intval($ordTodayRefRow['cnt'] ?? 0);
        if ($ordTodayAmt > $todayRefunds) {
            $todayRefunds = $ordTodayAmt;
            $todayRefundCount = $ordTodayCnt;
        }
    } catch (Exception $e) {}
