...
$mobile = getParam('mobile') ?: getParam('user_id') ?: '9876543210';

try {
    $stmtRzp = $pdo->query("SELECT `value` FROM store_settings WHERE `key` = 'enable_razorpay'");
    if ($stmtRzp && $rRzp = $stmtRzp->fetch(PDO::FETCH_ASSOC)) {
        if ($rRzp['value'] === '0') {
            sendJson(['status' => 'error', 'message' => 'Online payment via Razorpay is temporarily disabled by store admin.'], 400);
            exit();
        }
    }
} catch (Exception $e) {}
